...
Panel {
    ...
    PanelKeyCatcher {
      ...
      ScrollView {
        ...
        Column {
          ...
          // ---- Output: slider row, then output devices ----
          G.Group {
            id: outputGroup
            title: "Output"
            ...
            G.CursorSurface {
              ...
              G.PanelSlider {
                ...
                anchors.right: outputField.left
              ...
              G.ValueField {
                ...
                anchors.verticalCenter: parent.verticalCenter
                value: String(Math.round((outputSlider.dragging ? outputSlider.liveValue : root.outputVolume) * 100))
                unit: "%"
                opacity: root.outputMuted ? 0.5 : 1.0
              }
          ...
          // ---- Input: slider row with level meter, then input devices ----
          G.Group {
            id: inputGroup
            ...
            G.CursorSurface {
              ...
              visible: !!root.source
              width: parent.width
              height: inputControls.implicitHeight + 10
              ...
              Column {
                ...
                Item {
                  ...
                  G.ValueField {
                    ...
                    anchors.right: parent.right
                    anchors.verticalCenter: parent.verticalCenter
                    value: String(Math.round((inputSlider.dragging ? inputSlider.liveValue : root.inputVolume) * 100))
                    unit: "%"
                    opacity: root.inputMuted ? 0.5 : 1.0
                  }
                }

                // Live input level: 4px surface-3 track, text-3 fill (not
                // accent: the level is a reading, not a choice).
                Rectangle {
          ...
          G.Group {
            ...
            title: "Sources"
            count: String(root.displayAudioStreams.length)
  ...
  // Device row (output or input): icon 15px in text-3, device name as a
  // label, "active" status text on the chosen device. The cursor lifts the
  // row; the lifted row's icon turns accent and its text text-1. Mouse hover
  // updates the panel cursor at the root; visuals come entirely from
  // hasCursor via CursorSurface, never from containsMouse.
  component DeviceRowContent: Item {
    id: deviceContent
    ...
    Text {
      id: deviceIcon
      textFormat: Text.PlainText
      text: deviceContent.glyph
      color: deviceContent.hot ? G.Tokens.accent : G.Tokens.text3
      font.family: G.Tokens.valueFont
      font.pixelSize: 15
      width: 18
    ...
    G.Label {
      text: deviceContent.label
      color: deviceContent.hot ? G.Tokens.text1 : G.Tokens.text2
      anchors.left: deviceIcon.right
      anchors.leftMargin: 10
      anchors.right: deviceStatus.visible ? deviceStatus.left : parent.right
  ...
  component SinkRow: G.CursorSurface {
    id: sinkRow
    ...
    DeviceRowContent {
      glyph: root.sinkGlyph(sinkRow.node)
      label: root.nodeLabel(sinkRow.node)
      active: sinkRow.isActive
      hot: sinkRow.hasCursor
    }
  ...
  component SourceRow: G.CursorSurface {
    id: sourceRow
    required property var node
    required property int rowIndex
    ...
    DeviceRowContent {
      glyph: root.sourceGlyph(sourceRow.node)
      label: root.nodeLabel(sourceRow.node)
      active: sourceRow.isActive
      hot: sourceRow.hasCursor
    }
  ...
  // First line: mute icon, app name, "active" when it is the active media
  // player. Second line: slider with a value field on the right. h/l from
  // the keyboard adjusts THIS stream's volume (not the global output) when
  // the cursor sits on this row. Enter/Space mutes the stream.
  component StreamRow: G.CursorSurface {
    id: streamRow
    required property var node
    required property int rowIndex
    ...
    current: isActive
    implicitHeight: streamColumn.implicitHeight + 10
    ...
    Column {
      ...
      Item {
        ...
        Text {
          ...
          color: streamRow.hasCursor ? G.Tokens.accent : G.Tokens.text3
          font.family: G.Tokens.valueFont
          font.pixelSize: 15
          width: 18
          horizontalAlignment: Text.AlignHCenter
        ...
        G.Label {
          ...
          color: streamRow.hasCursor ? G.Tokens.text1 : G.Tokens.text2
          anchors.left: streamMuteIcon.right
          anchors.leftMargin: 10
          anchors.right: streamStatus.visible ? streamStatus.left : parent.right
          anchors.rightMargin: 8
          anchors.verticalCenter: parent.verticalCenter
        }
      ...
      Item {
        width: parent.width
        height: streamField.implicitHeight

        G.PanelSlider {
          id: streamSlider
          bar: root.bar
          anchors.left: parent.left
          anchors.right: streamField.left
          anchors.rightMargin: 10
          anchors.verticalCenter: parent.verticalCenter
          minimum: 0
          maximum: 1.5
        ...
        G.ValueField {
          ...
          anchors.right: parent.right
          anchors.verticalCenter: parent.verticalCenter
          value: String(Math.round((streamSlider.dragging ? streamSlider.liveValue : streamRow.streamVolume) * 100))
          unit: "%"
          opacity: streamRow.streamMuted ? 0.5 : 1.0
        }
      }
