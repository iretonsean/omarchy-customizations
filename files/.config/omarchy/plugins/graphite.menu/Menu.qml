...
Item {
  ...
  // Set by `omarchy-menu-select -- --columns <separator>`: each label splits at
  // the separator into two columns, so rows line up in any font, not only in
  // a monospace one. The width is the widest first part, measured once per open.
  property string dmenuColumnSeparator: ""
  property int dmenuColumnWidth: 0
  ...
  // Splits a select label at the column separator. The separator's own mark,
  // such as the arrow in "SUPER + K → Keybindings", leads the second column.
  function dmenuColumns(label) {
    var separator = root.dmenuColumnSeparator
    if (!separator) return null
    var at = label.indexOf(separator)
    if (at < 0) return null
    var mark = separator.trim()
    var rest = label.slice(at + separator.length).trim()
    return { lead: label.slice(0, at).trim(), rest: mark ? mark + " " + rest : rest }
  }

  // Graphite: the installed omarchy-menu-select cannot pass --columns yet,
  // so detect the keybindings layout ("SUPER + K    → Keybindings") here.
  // When most options contain " → ", split there.
  function detectDmenuColumns() {
    var separator = " → "
    var total = root.dmenuOptions.length
    if (total === 0) return ""
    var hits = 0
    for (var i = 0; i < total; i++) {
      if (String(root.dmenuOptions[i] || "").indexOf(separator) >= 0) hits += 1
    }
    return hits / total >= 0.8 ? separator : ""
  }

  // Measure every option, not only the filtered ones, so the column holds
  // still while the user types.
  function measureDmenuColumns() {
    var widest = 0
    for (var i = 0; root.dmenuColumnSeparator && i < root.dmenuOptions.length; i++) {
      var parts = String(root.dmenuOptions[i] || "").split("\t")
      var columns = root.dmenuColumns(parts.length > 1 ? parts[1] : parts[0])
      if (!columns) continue
      columnMetrics.text = columns.lead
      widest = Math.max(widest, columnMetrics.advanceWidth)
    }
    root.dmenuColumnWidth = Math.ceil(widest)
  }
  ...
  function openDmenu(payload) {
    ...
    dmenuColumnSeparator = String(payload.columnSeparator || "") || detectDmenuColumns()
    measureDmenuColumns()
    activeMenu = "root"
    navStack = []
    filterText = ""
