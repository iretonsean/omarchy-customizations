...
Panel {
  ...
  // A row closes a section when it is the last of its kind in the flat list.
  // The delegate uses this to round the bottom of the group background.
  function scrollSectionEnds(index) {
    var rows = scrollRows
    if (index < 0 || index >= rows.length) return false
    return index === rows.length - 1 || rows[index + 1].section !== rows[index].section
  }
      ...
      Column {
        ...
        Item {
          ...
          Text {
            ...
            text: root.icon
            color: root.adapter && root.adapter.enabled ? G.Tokens.text3 : G.Tokens.text4
          ...
          Column {
            ...
            G.PanelMeta {
              id: heroStatus
              text: root.heroStatusText.toLowerCase()
              width: parent.width
            }
          }
        }

        // Connected devices sit above the scroll area in their own group.
        G.Group {
          id: connectedList
          visible: root.connectedDevices.length > 0
          width: parent.width
          title: "Connected"
          count: String(root.connectedDevices.length)
          ...
          Repeater {
            ...
            DeviceRow {
              ...
              width: connectedList.width - G.Tokens.padGroup * 2
              dev: modelData
              rowIndex: index
        ...
        //
        // Paired and Available are groups, but their rows share one ListView
        // so the cursor and scrolling keep working. Each delegate draws its
        // own slice of the group background (surface-2): the first row of a
        // section adds the group name line and the top corners, the last row
        // adds the bottom corners and the gap to the next group.
        ListView {
          id: deviceListView
          // Rounded clip: overflow is cut with the group radius, not square.
          layer.enabled: true
          layer.effect: G.RoundedClip {}
          visible: root.scrollRows.length > 0
          width: parent.width
          ...
          delegate: Item {
            ...
            readonly property bool opensGroup: sectionTitle !== ""
            readonly property bool closesGroup: root.scrollSectionEnds(index)

            width: ListView.view.width
            height: groupBody.height + (closesGroup && !isLastRow ? G.Tokens.gapGroup : 0)
            readonly property bool isLastRow: index === root.scrollRows.length - 1

            // Group background slice. The rectangle runs past the clip on
            // the sides that continue into the next or previous row, so only
            // the group's outer corners are round.
            Item {
              id: groupBody
              width: parent.width
              height: sliceColumn.implicitHeight
                + (slice.opensGroup ? G.Tokens.padGroup : 0)
                + (slice.closesGroup ? G.Tokens.padGroup : G.Tokens.gapRow)
              clip: true

              Rectangle {
                width: parent.width
                y: slice.opensGroup ? 0 : -radius
                height: parent.height + (slice.opensGroup ? 0 : radius) + (slice.closesGroup ? 0 : radius)
              ...
              Column {
                id: sliceColumn
                x: G.Tokens.padGroup
                y: slice.opensGroup ? G.Tokens.padGroup : 0
                width: parent.width - G.Tokens.padGroup * 2
                spacing: G.Tokens.gapRow

                // Group name line: name left, mono count right.
                Item {
                  visible: slice.opensGroup
                  width: parent.width
                  ...
                  G.PanelMeta {
                    ...
                    anchors.verticalCenter: parent.verticalCenter
                    text: String(root.sectionCount(slice.modelData.section))
                  }
                }

                DeviceRow {
                  width: parent.width
                  dev: slice.modelData.dev
                  rowIndex: slice.modelData.indexInSection
                  sectionName: slice.modelData.section
                  isDiscovered: slice.modelData.section === "discovered"
                }
              }
            }
          }
  ...
  // One-line navigation row: icon, device name, and live state at the right
  // (mono status text or value). Pending state is owned by the panel so it
  // survives rows moving between sections.
  component DeviceRow: G.CursorSurface {
    id: row
    ...
    // "connected" is a state and shows as ok status text; pending actions
    // and the battery level are plain mono values.
    readonly property bool statusIsOk: statusText === "connected"
    ...
    Item {
      ...
      anchors.fill: parent
      anchors.leftMargin: 8
      anchors.rightMargin: forgetBtn.visible ? 4 : 8
      ...
      Row {
        ...
        G.Value {
          visible: row.statusText !== "" && !row.statusIsOk
          text: row.statusText
          anchors.verticalCenter: parent.verticalCenter
        }
