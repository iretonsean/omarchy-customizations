...
Panel {
  ...
  // CursorSurface for the shared chrome shared by rows and pills. Graphite:
  // the cursor is the neutral lift (G.CursorSurface / G.Button), so the
  // panel no longer derives hover and selected fills from the bar colors.
  ...
  // Graphite group name in sentence case ("Known networks"). Model.js keeps
  // Omarchy's upper-case strings so it stays identical to the original.
  function wifiGroupName(index) {
    var t = wifiSectionTitle(index)
    return t === "" ? "" : t.charAt(0) + t.slice(1).toLowerCase()
  }

  // Number of rows in the group that starts at `index` (known or other).
  function wifiGroupCount(index) {
    var net = wifiNetworks[index]
    if (!net) return 0
    var n = 0
    for (var i = 0; i < wifiNetworks.length; i++) {
      if (wifiNetworks[i] && !!wifiNetworks[i].known === !!net.known) n++
    }
    return n
  }
    ...
    Column {
      ...
      Item {
        ...
        Text {
          ...
          text: root.icon
          color: root.networkManagerAvailable ? G.Tokens.text3 : G.Tokens.text4
        ...
        Column {
          ...
          anchors.right: parent.right
          anchors.rightMargin: heroActions.width > 0 ? heroActions.width + 12 : 0
          anchors.verticalCenter: parent.verticalCenter
          spacing: 2

          // Link detail rides inline after the name — "Ethernet 2.5gbit" —
          // rather than in a pill, which crowded the on/off switch. The name
          // is a label (SF Pro); the link speed is a value (SF Mono).
          Row {
            id: heroSsid
            width: parent.width
            spacing: 6
            ...
            G.PanelTitle {
              id: heroTitle
              text: heroSsid.title
              width: Math.min(implicitWidth, heroSsid.width - (heroDetail.visible ? heroDetail.implicitWidth + heroSsid.spacing : 0))
            }
          ...
          G.PanelMeta {
            ...
            text: {
              if (root.info.type === "wifi") {
                if (root.canDisconnect) return root.connectionPhrase.toLowerCase()
                if (root.kind === "disconnected") return "not connected"
                return ""
              }
              if (root.info.type === "ethernet") return root.connectionPhrase.toLowerCase()
              if (root.kind === "disconnected") return "not connected"
              return ""
            }
            visible: text !== ""
      ...
      // Graphite: one "Connection" group. Two label/value pairs per line keep
      // the panel short; each line is a 30px data row with 8px side padding.
      G.Group {
        visible: !!root.info.iface
        width: parent.width
        title: "Connection"
        count: root.info.iface || ""

        GridLayout {
          x: 8
          width: parent.width - 16
          // One stat per line: label left, value at the right edge (data row).
          columns: 2
          columnSpacing: 20
          rowSpacing: G.Tokens.gapRow
          ...
          DetailValue {
            ...
            color: root.internetPingPacketLoss > 0 ? G.Tokens.error : G.Tokens.text3
          }
      ...
      // Graphite: a "Wi-Fi band" group. The count shows the live band; the
      // Automatic switch is the first row, then the band choices.
      G.Group {
        visible: root.canSelectBand
        width: parent.width
        title: "Wi-Fi band"
        count: root.bandLabel(root.bandCurrent)

        // Automatic qualifies the whole group, so it is the first row.
        Item {
          width: parent.width
      ...
      // DNS provider selection: a "DNS provider" group of four choices.
      G.Group {
        width: parent.width
        title: "DNS provider"
      ...
      ListView {
        ...
        //
        // Graphite: "Known networks" and "Other networks" are groups. The
        // section header stays inside the delegate (so scrolling, clipping
        // and positionViewAtIndex keep working); each delegate paints its
        // part of the surface-2 group background. The first row of a group
        // carries the group name line and the top corners, the last row the
        // bottom corners and the 10px gap to the next group.
        delegate: Item {
          id: netDelegate
          required property var modelData
          required property int index
          readonly property string sectionTitle: root.wifiGroupName(index)
          readonly property bool firstInGroup: sectionTitle !== ""
          readonly property bool lastInGroup: index === root.wifiNetworks.length - 1
            || root.wifiSectionTitle(index + 1) !== ""
          readonly property bool lastOverall: index === root.wifiNetworks.length - 1
          width: ListView.view.width
          height: groupFill.height + (lastInGroup && !lastOverall ? G.Tokens.gapGroup : 0)

          Rectangle {
            id: groupFill
            width: parent.width
            height: delegateColumn.implicitHeight
              + (netDelegate.firstInGroup ? G.Tokens.padGroup : 0)
              + (netDelegate.lastInGroup ? G.Tokens.padGroup : G.Tokens.gapRow)
            color: G.Tokens.surface2
            topLeftRadius: netDelegate.firstInGroup ? G.Tokens.radiusGroup : 0
            topRightRadius: netDelegate.firstInGroup ? G.Tokens.radiusGroup : 0
            bottomLeftRadius: netDelegate.lastInGroup ? G.Tokens.radiusGroup : 0
            bottomRightRadius: netDelegate.lastInGroup ? G.Tokens.radiusGroup : 0
          }

          Column {
            id: delegateColumn
            x: G.Tokens.padGroup
            y: netDelegate.firstInGroup ? G.Tokens.padGroup : 0
            width: parent.width - G.Tokens.padGroup * 2
            spacing: G.Tokens.gapRow

            // Group name line: name left, mono count right (as G.Group).
            Item {
              visible: netDelegate.firstInGroup
              width: parent.width
              height: visible ? G.Tokens.rowGroupName : 0

              G.PanelSectionHeader {
                text: netDelegate.sectionTitle
                topPadding: 0
                anchors.left: parent.left
                anchors.leftMargin: 8
                anchors.verticalCenter: parent.verticalCenter
              ...
              G.PanelMeta {
                text: String(root.wifiGroupCount(netDelegate.index))
                anchors.right: parent.right
  ...
  component BandPill: G.Button {
    ...
    // "2.4ghz" is a value: SF Mono. Choices sit on surface-1 in the group.
    fontFamily: G.Tokens.valueFont
    fontSize: G.Tokens.sizeValue
    background: G.Tokens.surface1
  ...
  component DnsProviderPill: G.Button {
    id: pill
    required property string provider
    required property int index

    text: provider
    // Provider names are labels (SF Pro). Choices sit on surface-1 in the
    // group; the chosen provider gets the accent-soft fill.
    background: G.Tokens.surface1
  ...
  // disconnects.
  component NetworkRow: G.CursorSurface {
    id: row
    ...
    // Graphite status text (SF Mono, lower case) at the right of the row:
    // "connected" in ok, failures in error, work in progress in text-3.
    readonly property string statusText: {
      ...
      if (isFailed) return (root.failureReason || "Failed").toLowerCase()
      if (isConnected) return "connected"
      return ""
    }

    readonly property color statusColor: {
      if (isFailed) return G.Tokens.error
      if (isBusy) return G.Tokens.text3
      return G.Tokens.ok
    }
    ...
    // Navigation row: 36px, 8px side padding, 15px icon, 10px to the label.
    Item {
      id: rowBody
      anchors.left: parent.left
      anchors.right: parent.right
      ...
      implicitHeight: G.Tokens.rowNav
      ...
      Item {
        ...
        // Keyboard cursor on Forget: the lift, no outline.
        Rectangle {
          anchors.fill: parent
        ...
        Text {
          ...
          color: row.forgetVisible ? G.Tokens.error : G.Tokens.text3
          font.family: G.Tokens.valueFont
          font.pixelSize: 15
        }
      ...
      // Status on the row line ("connected", "connecting…", a failure), not
      // on a second line.
      G.StatusText {
        id: rowStatus
        text: row.statusText
        visible: row.statusText !== ""
        status: row.isFailed ? "error" : "ok"
        color: row.statusColor
        anchors.right: rightAction.visible ? rightAction.left : parent.right
        anchors.rightMargin: rightAction.visible ? 6 : 4
        anchors.verticalCenter: parent.verticalCenter
      }
      ...
      G.Label {
        ...
        anchors.leftMargin: 10
        anchors.right: rowStatus.visible ? rowStatus.left : (rightAction.visible ? rightAction.left : parent.right)
        anchors.rightMargin: 8
        anchors.verticalCenter: parent.verticalCenter
      }
    }
    ...
    Item {
      ...
      anchors.rightMargin: 4
      anchors.topMargin: 2
      implicitHeight: (idField.visible ? idField.implicitHeight + 4 : 0) + pwField.implicitHeight + 6
      height: implicitHeight

      // Graphite field: surface-1, 5px radius, no outline.
      TextField {
        id: idField
        ...
        foreground: G.Tokens.text2
        selectionTint: Qt.rgba(G.Tokens.accent.r, G.Tokens.accent.g, G.Tokens.accent.b, 0.4)
        selectedTextColor: G.Tokens.text1
        placeholderTextColor: G.Tokens.text3
        leftPadding: 8
        rightPadding: 8
        topPadding: 6
        bottomPadding: 6
        background: Rectangle { color: G.Tokens.surface1; radius: G.Tokens.radiusField }
        enabled: !row.isBusy
      ...
      Rectangle {
        ...
        G.StatusText {
          ...
          status: row.isFailed ? "error" : "ok"
          color: row.isFailed ? G.Tokens.error : G.Tokens.text3
        }
      }

      // Right-anchored to line up with lockIndicator above. Esc closes
  ...
  // Graphite values: SF Mono 12px in text-3. Copyable values (IP address,
  // gateway) turn text-1 under the pointer.
  component DetailValue: InfoValue {
    ...
    color: valueMouse.enabled && valueMouse.containsMouse ? G.Tokens.text1 : G.Tokens.text3
  ...
  // Graphite label in a 30px data row: SF Pro 13px in text-2.
  component InfoLabel: G.Label {
    Layout.preferredHeight: G.Tokens.rowData
    verticalAlignment: Text.AlignVCenter
    elide: Text.ElideNone
  }

  component InfoValue: G.Value {
    Layout.preferredHeight: G.Tokens.rowData
    verticalAlignment: Text.AlignVCenter
  }
}
